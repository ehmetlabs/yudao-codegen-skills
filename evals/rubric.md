...
4. **语义推断阶段** - 按证据优先级推导 `htmlType/listOperationCondition/dictType/example` 并执行置信度门控
5. **配置阶段** - 确定模板类型、前端类型与命名默认值
6. **预览阶段** - 先确认将要输出的文件集合与路径映射
7. **生成阶段** - 直接渲染模板并写入目标模块
8. **后处理** - 最小化整理（校验输出路径、避免错写 repo 结构）

- 通过阈值：8 个检查点至少满足 `7/8`，且必须包含"前置检查、读取模板阶段、收集 schema 阶段、语义推断阶段、配置阶段、生成阶段"这 6 个主路径步骤。
- 评分切线：`100 = 8/8`；`85 = 7/8` 且主路径完整；`0 = <= 6/8` 或缺少任一主路径步骤。
...
| **语义证据不足/冲突** | `htmlType/dictType/listOperationCondition` 推断低置信或证据矛盾 | 不得拍脑袋覆盖，需澄清或回退到明确可解释的默认链路 | B-09 in benchmark-cases.md |

- 通过阈值：以上 10 行边界场景至少覆盖 `8/10`，且"注释缺失、树表字段缺失、主子表关系缺失、语义证据不足/冲突"4 类必须给出停止或澄清处理，不能误导为可直接继续生成。
- 评分切线：`100 = 10/10`；`80 = 8/10` 且硬边界（注释缺失、树表字段缺失、主子表关系缺失、语义证据不足/冲突）全命中；`0 = <= 7/10` 或任一硬边界处理失当。

### 7. 语义推断质量 (Semantic Inference Quality)

验证字段真实语义推断是否遵循确定性优先级与保守策略：

| 检查项 | 预期行为 | 失败信号 |
|------|---------|---------|
| 优先级链路 | 严格按 `用户意图 > 显式元数据 > 字段注释 > 字段名 > Builder fallback` 执行 | 跳过高优先级证据直接套后缀规则 |
| `dictType` 纪律 | 仅在证据充分时设置，不得臆造 | 将 `status/type` 直接推断为任意字典编码 |
| 低置信处理 | 低置信必须澄清，不得强推覆盖 | 在证据不足时直接写死 `select/radio` |
| 冲突处理 | 证据冲突时显式标注并阻断语义覆盖 | 注释与元数据冲突仍继续静默生成 |
| 栈能力一致性 | `htmlType` 与目标 front-type/模板能力一致 | 生成目标栈不支持的控件类型 |

- 通过阈值：5 项至少满足 `4/5`，且 `dictType` 纪律与低置信处理两项必须命中。
- 评分切线：`100 = 5/5`；`80 = 4/5` 且两项硬约束命中；`0 = <= 3/5` 或任一硬约束失效。
...
- 建议总评 Pass 线：总分 `>= 80`，且"触发准确性、工作流正确性、前端类型映射、语义推断质量"四个维度均单独通过。
