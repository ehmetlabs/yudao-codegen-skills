...
字段校验/默认推导 | `validateTableInfo` + `CodegenBuilder.buildTable/buildColumns` 提供表/字段约束和默认值 | CodegenServiceImpl.java | 注释完整性、命名、UI 类型和示例值都来自源码规则；Builder 规则是 fallback
字段语义推断与门控 | 对 `htmlType/listOperationCondition/dictType/example` 按「用户意图 > 显式元数据 > 字段注释 > 字段名 > Builder fallback」推断，并执行置信度门控 | SKILL.md (`Phase 4: Semantic Inference Contract`) | 低置信/冲突必须澄清，`dictType` 禁止臆造
